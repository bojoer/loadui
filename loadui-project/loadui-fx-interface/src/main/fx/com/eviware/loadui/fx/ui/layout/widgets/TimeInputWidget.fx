...
import com.eviware.loadui.fx.ui.form.fields.TimeField;
...
public class TimeInputWidget extends FormFieldWidget {	
	var timeField: TimeField;
	
	override var plc on replace {
		value = plc.getProperty().getValue();
		if(not FX.isInitialized(timeField)) {
			timeField = TimeField {
				value: value
				label: label
			}
	...
	override var field = bind timeField;
}
