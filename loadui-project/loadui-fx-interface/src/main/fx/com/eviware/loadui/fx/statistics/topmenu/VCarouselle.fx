...
public class VCarouselle extends CustomNode, Resizable, Pagination {
	...
	public var topArrowActive: String = "{__ROOT__}images/small_arrow_up.fxz";
	public var topArrowInactive: String = "{__ROOT__}images/small_arrow_up_inactive.fxz";
	public var bottomArrowActive: String = "{__ROOT__}images/small_arrow_down.fxz";
	public var bottomArrowInactive: String = "{__ROOT__}images/small_arrow_down_inactive.fxz";
	
	override function create():Node {
		def glow = Glow { level: .5 };
		def pagesLabel: Label = Label {
			text: bind "{page + 1} of {numPages}"
			layoutX: bind width - pagesLabel.layoutBounds.width
			layoutY: bind upArrow.layoutY
		...
		Group {
			content: [
				// following rectangle keeps width when there are no items 
				Rectangle{width: bind width, height: 5, fill: Color.TRANSPARENT },
				Group {
					content: [
						upArrow = Group {
