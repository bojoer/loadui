...
import com.eviware.loadui.fx.ui.layout.widgets.SelectWidget;
...
public class SelectFactory extends WidgetFactory {
		...
		if( lc instanceof PropertyLayoutComponent ) {
			...
			if( plc.has("options") )
				return PropertyLayoutComponentNode {
					layoutComponent: plc
					widget:SelectWidget { plc: plc }
				}
		}
		
		throw new WidgetCreationException("The ComboBox widget requires a PropertyLayoutComponent with a parameter 'options'!");
	}
}
