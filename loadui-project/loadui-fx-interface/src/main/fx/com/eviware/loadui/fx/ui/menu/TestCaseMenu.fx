...
public class TestCaseMenu extends HBox {
	...
	def switchTestcaseSubmenu: Menu = Menu {
		...
		onShowing: function() {
			...
			var sceneCollection = testCase.getProject().getChildren();
					...
					HBox {
						...
						content: [
							menuButton = MenuButton {
								...
								action: function():Void { StatisticsWindow.getInstance().show() }
								...
								action: function():Void { openURL("http://loadui.org/interface/project-view.html#2-scenario-view") }
							}, MenubarButton {
								...
								tooltip: Tooltip { text: ##[CLOSE_TESTCASE]"Close Scenario" }
								action: function():Void {
									MainWindow.instance.testcaseCanvas.generateMiniatures();
									AppState.byName("MAIN").setActiveCanvas( testCase.getProject() ) 
								}
