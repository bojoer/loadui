...
blacklist = ['org.w3c.', 'javax.', 'java.']
packages = []

def lib = new File(System.getenv('JAVAFX_HOME')+File.separator+'lib')
new File(lib, 'shared').eachFileRecurse {
    scanFile(it)
}
new File(lib, 'desktop').eachFileRecurse {
    scanFile(it)
}
